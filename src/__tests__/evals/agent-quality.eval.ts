...
describe("Agent Instructions Quality", () => {
  ...
  describe("Supplier Agent Instructions", () => {
    it("should reflect supplier personality for Supplier 1", () => {
      const instructions = buildSupplierInstructions(1);

      expect(instructions).toContain(SUPPLIER_CHARACTERISTICS[1].name);
      expect(instructions).toContain("propose-offer");
    });

    it("should reflect supplier personality for Supplier 2", () => {
      const instructions = buildSupplierInstructions(2);

      expect(instructions).toContain(SUPPLIER_CHARACTERISTICS[2].name);
    });

    it("should reflect supplier personality for Supplier 3", () => {
      const instructions = buildSupplierInstructions(3);

      expect(instructions).toContain(SUPPLIER_CHARACTERISTICS[3].name);
    });

    it("should include negotiation flexibility guidance", () => {
      const instructions = buildSupplierInstructions(1);

      expect(instructions).toContain("discount");
    });
  });
});
