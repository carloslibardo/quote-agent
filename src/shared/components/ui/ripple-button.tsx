...
function RippleButton({
  ...
  variant,
  size,
  scale = 6, // Base's "playful restraint": reduced from 10 to 6
  transition = {
    duration: 0.24, // Base's snappy pace: 240ms (was 600ms)
    ease: [0.4, 0, 0.2, 1], // Base's standard easing curve
  },
  ...props
}: RippleButtonProps) {
  ...
  const createRipple = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      // Base's snappy pace: 240ms max (was 600ms)
      setTimeout(() => {
  ...
  return (
    <motion.button
      ...
      onClick={handleClick}
      whileTap={{ scale: 0.98 }} // Base's "playful restraint": 2% instead of 5%
      whileHover={{ scale: 1.02 }} // Base's "playful restraint": 2% instead of 5%
